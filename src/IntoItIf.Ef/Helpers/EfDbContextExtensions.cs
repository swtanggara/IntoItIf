         ...
         return dataContext
            .MapAsync(
               x =>
               {
                  ...
                  return x.Database.ExecuteSqlCommandAsync(inSql, inCtok, inParameters);
               });
      }
         ...
         return dataContext
            .Map(
               x =>
               {
                  var inSql = sql.ReduceOrDefault();
                  var inParameters = parameters.ToOptionOfEnumerable().ReduceOrDefault();
                  return x.Set<T>().SqlQuery(inSql, inParameters).ToList();
               });
      }
         ...
         return dataContext
            .MapAsync(
               x =>
               {
                  ...
                  var inParameters = parameters.ToOptionOfEnumerable().ReduceOrDefault();
                  var inCtok = ctok.Reduce(CancellationToken.None);
                  return x.Set<T>().SqlQuery(inSql, inParameters).ToListAsync(inCtok);
               });
      }
