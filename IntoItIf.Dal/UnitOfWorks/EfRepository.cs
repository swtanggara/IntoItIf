...
namespace IntoItIf.Dal.UnitOfWorks
{
   ...
   internal sealed class EfRepository<T> : BaseRepository<T>
      where T : class
   {
      #region Fields
      ...
      public EfRepository(Option<IDbContext> dbContext) : this(dbContext, None.Value)
      {
      }

      public EfRepository(Option<IDbContext> dbContext, Option<IQueryable<T>> customQuery) : base(
         dbContext,
         customQuery)
      {
      ...
      public override Option<Task<T>> FindByKeysAsync(Option<object>[] keyValues, Option<CancellationToken> ctok)
      {
         return keyValues.ToOptionOfEnumerable()
            ...
            .Map(x => _dbSet.FindAsync(x.Item2, x.Item1.ToArray()));
      }
         ...
         try
         {
            ...
            if (!(map is Some<T>))
               return Fail<bool>.Throw(
                  new InvalidOperationException("Invalid processing result of System.Data.Entity.DbSet<T>.Add()"));
            return true;
         }
         catch (Exception ex)
         {
      ...
      protected override Option<bool> RemoveEntry(Option<T> exist)
      {
         try
         {
            ...
            if (!(map is Some<T>))
               return Fail<bool>.Throw(
                  new InvalidOperationException("Invalid processing result of System.Data.Entity.DbSet<T>.Remove()"));
