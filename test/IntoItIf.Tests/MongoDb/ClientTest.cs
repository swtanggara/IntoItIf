namespace IntoItIf.Tests.MongoDb
{
   ...
   public class ClientTest
   {
      ...
      public async Task ShouldConsumerConnect()
      {
         ...
         var _ = args.ToOption()
            .Map(x => (Args: x, ValidationResult: x.Validate().ReduceOrDefault()))
            .IfMapFlatten(
               x => !x.ValidationResult.IsValid,
               x => x.ValidationResult.ToFail<Dictionary<string, object>>())
            .ElseMapFlattenAsync(
               async x =>
               {
                  return await uow.GetRepository<MongoRepository<Consumer>, Consumer>()
                     .Combine(args.ToOption())
                     .Map(y => (Repo: y.Item1, Args: y.Item2))
                     .MapFlattenAsync(
                        async y =>
                        {
                           using (var trx = uow.GetDbTransaction<OpsBossesDataContext>())
                           {
                              try
                              {
                                 var inResult = await y.Repo.AddAsync(x.Args);
      ...
      public async Task ShouldPnrConnect()
      {
         ...
         var _ = args.ToOption()
            ...
            .ElseMapFlattenAsync(
               async x =>
               {
                  return await uow.GetRepository<MongoRepository<Pnr>, Pnr>()
                     .Combine(args.ToOption())
                     .Map(y => (Repo: y.Item1, Args: y.Item2))
                     .MapFlattenAsync(
                        async y =>
